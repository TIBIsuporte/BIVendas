...
app.post("/consulta", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Erro na API externa (Grid): Status ${response.status}`);
...
// Nova Rota para buscar Devoluções via ProdutosPorOS
app.post("/consultadevolucoes", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Erro na API externa (Devoluções): Status ${response.status}`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
